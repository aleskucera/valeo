...
# preprocessing
min_height = 0.3  # from the ground  # argoverse nema ground
max_height = 3.  # from the ground
max_dist = 10  # meters
min_dist = 2  # meters

DATA_PATH = '/home/ales/school/valeo/data'
...
def compensate_ego_motion(data):
    ...
    for i in range(len(pc1)):
        deformed_pc = (torch.cat([pc1[i], torch.ones_like(pc1[i][:, :1])], dim=1) @ data['relative_pose'][i].T.to(
            pc1.device))[None, :, :3]
        global_pc = (torch.cat([pc1[i], torch.ones_like(pc1[i][:, :1])], dim=1) @ poses[i].T.to(pc1.device))[None, :,
                    :3]
        global_pc2 = (torch.cat([pc2[i], torch.ones_like(pc2[i][:, :1])], dim=1) @ poses[i].T.to(pc1.device))[None, :,
                     :3]
...
class NSF_dataset():
    ...
    def __init__(self, root_dir='home/ales/school/valeo/data', dataset_type: str = 'argoverse', subfold='val'):
...
class SFDataset4D():
    ...
    def __getitem__(self, idx):
        ...
        data_dict['frame_path_list'] = frame_path_list
...
def bboxes_demo():
    ...
    from data.path_utils import get_inst_trajes
    from ops.box_utils import get_inst_bbox_sizes
    from data.PATHS import DATA_PATH
    from mayavi import mlab
    ...
    mlab.points3d(global_cloud[::10, 0], global_cloud[::10, 1], global_cloud[::10, 2], color=(0, 0, 1),
                  scale_factor=0.1)
    mlab.show()
