...
class Vis_function(Visual_PCL_Template):
    ...
    def update_scan(self):
        ...
        # breakpoint()
        # green flow
        if 'flow' in data.files or 'gt_flow' in data.files:
            all_rays = []
            flow_mask = np.linalg.norm(data['gt_flow'][:,:3], axis=1) > 0.05
            for x in range(1, int(20)):
                ray_points = data['pts'][flow_mask] + (data['gt_flow'][flow_mask]) * (x / int(20))
                all_rays.append(ray_points)

            all_rays = np.concatenate(all_rays).reshape(-1, 4)[:,:3]
