...
def get_rotations_from_yaw_angle(yaw):
    """
    Get rotation matrices from yaw angles.

    Args:
        yaw (torch.Tensor): Yaw angles.

    Returns:
        torch.Tensor: Rotation matrices.
    """

    rotations = torch.cat((torch.zeros(len(yaw), 2, device=yaw.device), yaw), dim=1)
